...
class _FreehandDrawingCanvasState extends State<FreehandDrawingCanvas> {
  ...
  // }
    void onPointerMove(PointerMoveEvent details) {
      ...
      if (line.value != null) {
        // Update the existing Stroke object instead of creating a new one
        var currentPoints = line.value!.points;
        currentPoints.add(point); // Add the new point to the existing list
        line.value = Stroke(List.from(currentPoints)); // Update the line with the new points list
      }
    }
...
class StrokePainter extends CustomPainter {
  ...
  Path generatePath(List<PointVector> points, StrokeOptions options) {
    ...
    for (final point in data.skip(2)) {
      // path.lineTo(point.dx, point.dy);
      path.quadraticBezierTo(point.dx, point.dy, point.dx, point.dy);
    }
